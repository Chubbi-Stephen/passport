  ...
  return (
    <div className="app-shell">
      ...
      <div className="app-main">
        <Topbar title="Past Questions Practice" subtitle="JAMB & WAEC archive — 50,000+ questions" />
        <div className="page-content">
          <div style={{ display:'grid', gridTemplateColumns:'280px 1fr', gap:24, height:'calc(100vh - var(--navbar-h) - 64px)', minHeight:600 }}>
              ...
              <div className="card" style={{ flex:1, padding:0, overflow:'hidden' }}>
                ...
                <div style={{ overflowY:'auto', maxHeight:300 }}>
                  {questions.map((q, i) => {
                    ...
                    return (
                      <button key={q.id} onClick={()=>setCurrent(i)} style={{ width:'100%', padding:'12px 16px', borderBottom:'1px solid var(--clr-border-light)', textAlign:'left', background: i===current ? 'var(--clr-primary-50)' : 'white', display:'flex', gap:10, alignItems:'center', cursor:'pointer', transition:'background var(--ease)', border:'none' }}>
                        ...
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ fontSize:'0.75rem', fontWeight:600, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', color: i===current?'var(--clr-primary)':'var(--clr-text-primary)' }}>{q.subject?.name} {q.year}</div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
              {loading ? (
                <div className="card" style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center' }}>Loading questions...</div>
              ) : q ? (
                <>
                  <div className="card" style={{ flex:1 }}>
                    <div style={{ display:'flex', gap:10, marginBottom:20, alignItems:'center', flexWrap:'wrap' }}>
                      ...
                      <span className="badge" style={{ background:'var(--clr-bg)', color:'var(--clr-text-muted)' }}>{q.topic}</span>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', gap:12, marginBottom:28 }}>
                      {q.options.map((opt, i) => {
                        ...
                        return (
                          <button key={i} onClick={() => handleSelect(q.id, opt)} style={{ padding:'14px 18px', borderRadius:'var(--r-md)', border:`1.5px solid ${border}`, background:bg, color, textAlign:'left', fontSize:'0.9375rem', cursor: isRevealed?'default':'pointer', transition:'all var(--ease)', fontWeight: isSelected||isCorrect&&isRevealed ? 600 : 400 }}>
                            {opt}
                          </button>
                        );
                      })}
                    ...
                    {revealed[q.id] && (
                      <div className="animate-fadeInUp" style={{ padding:'16px 20px', background:'var(--clr-primary-50)', borderRadius:'var(--r-md)', border:'1px solid var(--clr-primary-100)' }}>
                        <div style={{ fontWeight:700, color:'var(--clr-primary)', marginBottom:8, display:'flex', gap:8, alignItems:'center' }}>
                          <Lightbulb size={18} /> Why this is correct:
                        </div>
                ...
                <div className="card" style={{ display:'flex', alignItems:'center', justifyContent:'center', flex:1, flexDirection:'column', gap:16 }}>
                  <div style={{ color:'var(--clr-text-muted)', opacity:0.3 }}><SearchX size={64} /></div>
                  <h3 style={{ fontFamily:'var(--font-display)', fontWeight:700 }}>No questions found</h3>
                  <p style={{ color:'var(--clr-text-muted)' }}>Try changing filters or wait for content updates</p>
                </div>
              )}
            </div>
          </div>
