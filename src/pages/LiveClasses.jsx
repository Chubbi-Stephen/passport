...
export default function LiveClasses() {
  ...
  return (
    <>
      {/* Banner */}
      <div style={{ background:'linear-gradient(135deg, #071812, #0A6640)', borderRadius:'var(--r-xl)', padding:'32px', marginBottom:24, color:'white', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:20 }}>
        ...
        <div className="mobile-hidden" style={{ width:100, height:100, background:'rgba(255,255,255,0.1)', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', color:'var(--clr-accent)' }}>
          <Video size={48} />
        </div>
      </div>
      ...
      <div className="grid-2-dynamic" style={{ display:'grid', gridTemplateColumns:'1fr 300px', gap:24 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            {upcomingClasses.map(c => (
              <div key={c.id} className="card" style={{ display:'flex', alignItems:'center', gap:16, flexWrap:'wrap' }}>
                <div className="mobile-hidden" style={{ width:52, height:52, borderRadius:14, background:'var(--clr-primary-50)', display:'flex', alignItems:'center', justifyContent:'center', color:'var(--clr-primary)' }}>
                  <Users size={24} />
                </div>
                <div style={{ flex:1, minWidth:200 }}>
                  ...
                  <div style={{ fontSize:'0.8125rem', color:'var(--clr-text-muted)', marginTop:4 }}>with {c.tutor}</div>
                </div>
                <div style={{ textAlign:'right', minWidth:120 }}>
                  ...
                  <button className={`btn btn-sm ${c.status.includes('Live')?'btn-primary':'btn-outline'}`} style={{ borderRadius:999, gap:6 }}>
                    {c.status.includes('Live') ? 'Join Now' : 'Remind Me'}
                  </button>
                </div>
              </div>
            ))}
            ...
            {pastRecordings.map(r => (
              <div key={r.id} className="card card-hover" style={{ padding:16 }}>
                <div style={{ height:100, background:'var(--clr-bg)', borderRadius:'var(--r-md)', marginBottom:12, display:'flex', alignItems:'center', justifyContent:'center', color:'var(--clr-primary)', position:'relative' }}>
                  <Tv size={32} />
                  <span style={{ position:'absolute', bottom:6, right:6, background:'rgba(0,0,0,0.6)', color:'white', padding:'2px 6px', borderRadius:4, fontSize:'0.65rem' }}>{r.duration}</span>
                </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:24 }}>
          <div className="card" style={{ background:'var(--clr-accent-50)', border:'1px solid var(--clr-accent-light)' }}>
            ...
            <p style={{ fontSize:'0.8125rem', color:'var(--clr-text-secondary)', marginBottom:16, lineHeight:1.6 }}>Get 1-on-1 help from top tutors. Premium only.</p>
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {['Mathematics - Today 2pm','English - Wed 4pm'].map((t,i) => (
          ...
          <div className="card">
            <h3 style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:'1rem', marginBottom:16 }}>Your Stats</h3>
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <span style={{ fontSize:'0.8125rem', color:'var(--clr-text-muted)' }}>Attended</span>
