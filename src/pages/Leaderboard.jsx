    ...
    <>
      ...
      <div style={{ background:'linear-gradient(135deg, #071812, #0A6640)', borderRadius:'var(--r-xl)', padding:'24px 28px', marginBottom:24, display:'flex', alignItems:'center', gap:24, flexWrap:'wrap' }}>
        ...
        <div style={{ display:'flex', gap:28 }}>
          {[{v:'287',l:'Mock Score'},{v:'↑ 3',l:'Rank Change'}].map((s,i)=>(
            <div key={i}>
      ...
      <div className="grid-2-dynamic" style={{ display:'grid', gridTemplateColumns:'1fr 300px', gap:24 }}>
        <div>
          {/* Tabs */}
          <div style={{ display:'flex', gap:4, marginBottom:20, background:'var(--clr-bg-card)', borderRadius:'var(--r-md)', padding:4, border:'1px solid var(--clr-border-light)', overflowX:'auto' }}>
            {tabs.map((t,i)=>(
              <button key={i} onClick={()=>setActiveTab(i)} style={{ flex:1, padding:'9px 12px', borderRadius:'var(--r-sm)', fontWeight:600, fontSize:'0.8125rem', background:activeTab===i?'var(--clr-primary)':'transparent', color:activeTab===i?'white':'var(--clr-text-muted)', cursor:'pointer', border:'none', transition:'all var(--ease)', whiteSpace:'nowrap' }}>{t}</button>
            ))}
          </div>
            ...
            {[nationalBoard[1],nationalBoard[0],nationalBoard[2]].map((u,i)=>{
              ...
              return (
                <div key={u.rank} style={{ textAlign:'center', display:'flex', flexDirection:'column', alignItems:'center', gap:10 }}>
                  <div style={{ color: i===1?'var(--clr-accent)':i===0?'#C0C0C0':'#CD7F32' }}>{i===1 ? <Trophy size={28} /> : <Medal size={24} />}</div>
                  ...
                  <div style={{ fontWeight:700, fontSize:'0.8125rem', maxWidth:100 }}>{u.name.split(' ')[0]}</div>
                  ...
                  <div style={{ height:heights[i], width:80, background: i===1?'linear-gradient(180deg, var(--clr-accent), #e8961a)':i===0?'linear-gradient(180deg, #C0C0C0, #A0A0A0)':'linear-gradient(180deg, #CD7F32, #A0522D)', borderRadius:'12px 12px 0 0', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-display)', fontSize:'1.25rem', fontWeight:900, color:'white', boxShadow:'var(--shadow-md)' }}>
                    #{u.rank}
                  </div>
                </div>
              );
          ...
          <div className="card" style={{ padding:0, overflowX:'auto' }}>
            <div style={{ minWidth:600 }}>
              <div style={{ padding:'14px 20px', borderBottom:'1px solid var(--clr-border-light)', display:'grid', gridTemplateColumns:'50px 1fr 100px 80px 80px 60px', gap:12, fontSize:'0.7rem', fontWeight:700, color:'var(--clr-text-muted)', textTransform:'uppercase' }}>
                <span>Rank</span><span>Student</span><span>State</span><span>Score</span><span>Streak</span><span>+/-</span>
              </div>
              {nationalBoard.map(u => (
                <div key={u.rank} style={{ padding:'14px 20px', borderBottom:'1px solid var(--clr-border-light)', display:'grid', gridTemplateColumns:'50px 1fr 100px 80px 80px 60px', gap:12, alignItems:'center', background:u.isMe?'var(--clr-primary-50)':'white' }}>
                  <div style={{ fontFamily:'var(--font-display)', fontWeight:800, fontSize:'1.0625rem', color: u.rank<=3?'var(--clr-accent)':u.isMe?'var(--clr-primary)':'var(--clr-text-muted)' }}>
                    {u.badge || `#${u.rank}`}
                  </div>
                  <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                    <div className="avatar avatar-sm">{u.name.split(' ').map(n=>n[0]).join('')}</div>
                    <div style={{ fontWeight:u.isMe?700:500, fontSize:'0.875rem' }}>{u.name}</div>
                  </div>
                  <span style={{ fontSize:'0.75rem', color:'var(--clr-text-muted)' }}>{u.state}</span>
                  <span style={{ fontFamily:'var(--font-display)', fontWeight:700 }}>{u.score}</span>
                  <span style={{ fontSize:'0.8125rem' }}>{u.streak}d</span>
                  <span style={{ fontSize:'0.8125rem', color: u.change>0?'var(--clr-success)':u.change<0?'var(--clr-danger)':'var(--clr-text-muted)' }}>{u.change > 0 ? `+${u.change}` : u.change < 0 ? u.change : '-'}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
          <div className="card" style={{ padding:0 }}>
            <div style={{ padding:'14px 20px', borderBottom:'1px solid var(--clr-border-light)' }}>
              <h3 style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:'0.9375rem' }}>Subject Champions</h3>
            </div>
            {subjectLeaders.map((l,i) => (
              <div key={i} style={{ padding:'12px 16px', borderBottom:i===subjectLeaders.length-1?'none':'1px solid var(--clr-border-light)', display:'flex', alignItems:'center', gap:10 }}>
                <div className="avatar avatar-xs">{l.avatar}</div>
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:'0.75rem', fontWeight:600 }}>{l.subject}</div>
                  <div style={{ fontSize:'0.7rem', color:'var(--clr-text-muted)' }}>{l.name}</div>
                </div>
                <span style={{ fontWeight:700, color:'var(--clr-accent)', fontSize:'0.875rem' }}>{l.score}</span>
              </div>
            ))}
          </div>
